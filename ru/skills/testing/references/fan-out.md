...
3. **Синтез (оркестратор, без агента).** Собрать выводы, дедупить (один дефект часто приходит от двух осей), проставить/нормализовать severity, сопоставить с AC, отсеять дубли и не-дефекты.
...
- ЧИСЛА из отчёта субагента (размеры, отступы, gap, доли заполнения) — гипотеза до личной перепроверки по первоисточнику: агент пересказывает дамп, а в дампе размер вложенного растра легко принять за размер блока. Перед внесением числа в отчёт или дефект — замерить самому.
...
- **Возможности браузеров при нарезке пачек:** тач-жесты (свайпы) воспроизводимы только в Chromium через CDP `Input.dispatchTouchEvent`; WebKit-автоматизация обычно умеет лишь `tap`, а headless WebKit не двигает фокус по Tab. Мобильные жестовые ТК агенту на WebKit не назначать — получишь ложный Blocked; Blocked от агента со ссылкой на «нет инструмента» перепроверять на браузере с нужной возможностью.
