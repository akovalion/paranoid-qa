...
4. Покрытие:
   ...
   **Оверлеи/модалки/панели (пример пака под тип объекта):** блокировка прокрутки (позиция сохраняется, фон не скроллится, компенсация ширины скроллбара без «прыжка»), закрытие ×/Esc/клик по фону/Back, deep-link и перезагрузка (состояние в URL), даблклик/спам, ресайз при открытом, стекинг оверлеев. У других типов объектов — свой негатив-пак (формы, списки, навигация, API; см. references).
   Включай в покрытие:
   - Позитивные сценарии
   - Негативные сценарии
   - Граничные значения
