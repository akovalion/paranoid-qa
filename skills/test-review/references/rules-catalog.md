...
```
**Важно:** во многих конфигах нужных правил НЕТ (ни `@typescript-eslint/no-floating-promises`, ни `eslint-plugin-playwright/missing-playwright-await`) → линт пропустит floating promise; `valid-expect` ловит лишь часть форм с `expect`. Проверь конфиг проекта; если правил нет — **перечитывай глазами** и предложи добавить оба.
