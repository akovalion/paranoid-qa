...
- **Browser capabilities when slicing batches:** touch gestures (swipes) are only reproducible in Chromium via CDP `Input.dispatchTouchEvent`; WebKit automation usually offers only `tap`, and headless WebKit does not move focus on Tab. Do not assign mobile gesture TCs to an agent on WebKit — you will get a false Blocked; re-check any agent's Blocked that cites "no tool for this" on a browser that has the capability.
